"""
Vercel Python Serverless Function — /api/ask
مبني على نفس منطق server.py الأصلي، لكن بالصيغة اللي Vercel بيطلبها:
- لازم يكون في مسار /api
- لازم يكون فيه كلاس اسمه "handler" (بالظبط) بيرث من BaseHTTPRequestHandler
- من غير serve_forever() أو تشغيل سيرفر — Vercel هو اللي بيستدعي do_POST مباشرة لكل Request
"""
...
# ===== Rate Limiting =====
# ملاحظة مهمة: في بيئة serverless زي Vercel، كل Function ممكن تتشغّل في instance
# مختلفة أو تتصفّر مع كل cold start، فالـ rate limit ده بيشتغل بس كحماية تقريبية
# مش دقيقة 100%. لو عايز rate limiting موثوق استخدم Vercel KV أو Upstash Redis.
...
def _gemini_call(text, use_search=False):
    ...
    try:
        ...
        # نتجاهل أي جزء "thought" احتياطًا (حتى لو كان thinkingBudget=0)
